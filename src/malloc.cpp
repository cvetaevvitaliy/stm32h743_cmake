...
caddr_t _sbrk(int32_t incr) {
    ...
    incr = incr + z;
    // if heap size was overfloated
    if (current_ends + incr > &_heap_end) {
        errno = ENOMEM;
        return (caddr_t)(-1);
    }
...
void gluing_free_blocks(void *start, void *end) {
    ...
    while (curr < end) {
        // first block and nexteses
        if (mem->av == 0) {
            void *initial_addr = mem;
            uint16_t sizeses = 0; // initial adding size
            curr = mem;
            // go to next block
            mem = (MemBlock *)((uint8_t *)curr + mem->size + sizeof(MemBlock) + align(mem->size));
            curr = mem;
            while (mem->av == 0 && curr < end) {
                // sum all sizes available blocks goes straight
                sizeses += mem->size + align(mem->size) + sizeof(MemBlock);
                curr = mem;
                // go to next straight free block
                mem = (MemBlock *)((uint8_t *)curr + mem->size + sizeof(MemBlock));
                curr = mem;
            }
        ...
        // mem->av==1  -  go to next block
        else {
            curr = mem;
            mem = (MemBlock *)((uint8_t *)curr + mem->size + align(mem->size) + sizeof(MemBlock));
...
void *malloc(volatile size_t bytes) {
    ...
    if (curr == curr_end) {
        // if this condition => _sbrk was not called yet
        curr = init_malloc(bytes);
        mem = (MemBlock *)curr;
        mem->av = 1;
        mem->size = bytes;
        curr = (uint8_t *)curr + sizeof(MemBlock); // increase on struct size for first shift on memory start
    } else {
        // search first good condition
        curr = (uint8_t *)curr + sizeof(MemBlock); // for returning pointer on an allocated memory
        while (curr <= curr_end) {
            // go while current_end
            if ((mem->av == 0) && (mem->size >= bytes)) {
                // first suitable condition
                mem->av = 1;
                ...
                if (((old_size + incr_old) - (bytes + incr_new)) > sizeof(MemBlock) + 3) {
                    // adding new block
                    void *new_curr = (uint8_t *)curr + mem->size + incr_new; // start address of new block (alignment)
            ...
            curr = (uint8_t *)curr + mem->size + sizeof(MemBlock) + align(mem->size); // go to next block
            mem = (MemBlock *)((uint8_t *)curr - sizeof(MemBlock));                   // mem = curr<<3
        }
        curr = init_malloc(bytes); // if was not any available blocks
        mem = (MemBlock *)curr;
        mem->av = 1;
        mem->size = bytes;
        curr = (uint8_t *)curr + sizeof(MemBlock); // increase on struct size for pointer on allocated memory
    }
    return curr;
}
//+ gluing free blocks
void free(void *firstbyte) {
    mem = (MemBlock *)((uint8_t *)firstbyte - sizeof(MemBlock));
    mem->av = 0;
    void *curr_end = _sbrk(0);
    void *curr = &_heap_start;
